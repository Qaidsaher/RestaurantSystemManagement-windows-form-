...
namespace RestaurantSystemManagement
{
    public partial class Tables : Form
    {
        public Tables()
        {
            ...
            string[] strNames = { "في اي بي", "عادي" };
            string[] strType = { "كبيره", "متوسطه", "صغيره" };
            cbxName.Items.AddRange(strNames);
            cbxType.Items.AddRange(strType);
        ...
        private void BtnExcute_Click(object sender, EventArgs e)
        {
            if (cbxName.Text != "")
            {
                ...
                {
                    Id = Program.dbase.CountItem("Tables") + 1;
                    Program.dbase.Add("INSERT INTO Tables (TableName,TableCapacity)  VALUES ('" + cbxName.Text + "' ,'"+ cbxType.Text + "');");
                    cbxName.Text = "";
                    Id = -1;
                ...
                else
                {
                    ...
                    Program.dbase.Update("update Tables set TableName = '" + cbxName.Text + "' , TableCapacity = '" + cbxType.Text+ "' where  TableID = " + Id + ";");
                    Id = -1;
                    cbxType.Text = "";
                    RefreashDataGridView();
                }
        ...
        private void DataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                {
                    Id = int.Parse(dataGridView1.Rows[e.RowIndex].Cells["TablID"].Value.ToString());
                    if (DialogResult.Yes == MessageDialog.ShowDialog("delete"))
                    {
                        /*
                        Program.dbase.Delete("DELETE FROM Tables WHERE SectionID = " + Id + "");
                ...
                {
                    Id = int.Parse(dataGridView1.Rows[e.RowIndex].Cells["TablID"].Value.ToString());
                    cbxName.Text = dataGridView1.Rows[e.RowIndex].Cells["TableName"].Value.ToString();
                    cbxType.Text = dataGridView1.Rows[e.RowIndex].Cells["TableCapacity"].Value.ToString();
