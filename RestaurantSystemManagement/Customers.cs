        ...
        private void DataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                if (dataGridView1.Columns[e.ColumnIndex].Name == "حذف")
                {
                    ...
                    int empId = int.Parse(dataGridView1.Rows[e.RowIndex].Cells["Cust_ID"].Value.ToString());
                    if (DialogResult.Yes == MessageDialog.ShowDialog("delete"))
                    {
                        Program.dbase.Delete("DELETE FROM Customer WHERE Cust_ID = " + empId + "");
                    }
                ...
                else if (dataGridView1.Columns[e.ColumnIndex].Name == "تعديل")
                {
                    ...
                    CustomerDialog sd = new CustomerDialog(empId);
                    sd.ShowDialog();
                    RefreashDataGridView();
                }
            }
